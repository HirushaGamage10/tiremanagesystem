...
<body class="bg-gray-100 h-screen flex flex-col overflow-hidden">
  ...
  <div class="flex flex-1 overflow-hidden flex-col sm:flex-row">
    ...
    <main class="flex-1 overflow-y-auto p-4 sm:p-8 bg-[#999999]">
      ...
      <a href="{{ route('transport.viewtransport') }}" class="flex items-center space-x-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 mb-4 w-max">
      ...
      <div class="bg-white rounded-[30px] shadow p-4 sm:p-6 sm:px-16">
        <h2 class="text-xl sm:text-2xl font-extrabold mb-6 mt-2 text-blue-800">Transport Approval</h2>
        ...
        <form class="space-y-4" method="POST" action="{{ route('transport.transportapprovalsubmit', $request->id) }}">
          ...
          <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
              <label class="sm:w-1/3 text-sm font-medium mb-1 sm:mb-0">Request Code</label>
              <input type="text" class="flex-1 rounded-full bg-gray-200 px-4 py-2" value="{{ $request->request_code ?? '' }}" readonly />
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4">
            ...
            <div class="flex flex-col md:flex-row md:items-start md:space-x-4 space-y-2 md:space-y-0">
            <label class="md:w-1/3 font-medium text-sm mt-1">Mechanical Comments</label>
            <textarea class="flex-1 border rounded-lg px-4 py-2 bg-gray-200" rows="3" readonly>{{ $request->mechanicalApprovalCheck->mechanic_comments ?? '' }}</textarea>
            </div>
            <!-- Transport Officer Comments (editable) -->
            <div class="flex flex-col md:flex-row md:items-start md:space-x-4 space-y-2 md:space-y-0">
              <label class="md:w-1/3 font-medium text-sm mt-1">Transport Officer Comments</label>
              <textarea name="transport_comments" class="flex-1 border rounded-lg px-4 py-2 bg-gray-200" rows="3"></textarea>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-2 md:space-y-0">
              <label class="md:w-1/3 font-medium text-sm">Transport Officer Service No</label>
              <input type="text" name="transport_officer_number" class="flex-1 border rounded-full px-4 py-2 bg-gray-200" />
            </div>
            <!-- Uploaded Images -->
            @if($request->images)
              <div class="flex flex-col md:flex-row md:items-start md:space-x-4 space-y-2 md:space-y-0">
